...
func (c *Client) Latest(ctx context.Context, modulePath string) (*VersionInfo, error) {
	if cached, ok := c.cache.Get(modulePath + "@latest"); ok {
		if info, ok := cached.(*VersionInfo); ok {
			return info, nil
		}
	}

	url := fmt.Sprintf("%s/%s/@latest", c.baseURL, modulePath)
	...
	c.cache.Set(modulePath+"@latest", &info, 5*time.Minute)
...
func (c *Client) Versions(ctx context.Context, modulePath string) ([]string, error) {
	if cached, ok := c.cache.Get(modulePath + "@list"); ok {
	...
	c.cache.Set(modulePath+"@list", versions, 5*time.Minute)
...
func (c *Client) GetModFile(ctx context.Context, modulePath, version string) ([]byte, error) {
	...
	url := fmt.Sprintf("%s/%s/@v/%s.mod", c.baseURL, modulePath, version)
	req, err := http.NewRequestWithContext(ctx, "GET", url, nil)
	if err != nil {
		return nil, fmt.Errorf("creating request: %w", err)
	}
